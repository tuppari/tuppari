...
function createSignedRequestConfig(method, uri, operation, body, secretKey) {
  ...
  var headers = {
    ...
    "X-Tuppari-Date": ISODateString(now),
    "X-Tuppari-Operation": operation
  };

  var signedHeaders = createSignedHeaders(headers);
  var canonicalRequest = createCanonicalRequest(method, puri.pathname, puri.query, headers, body);
  var stringToSign = createStringToSign(canonicalRequest, now);
  var signature = createSignature(secretKey, stringToSign, now, puri.hostname);

  headers["Authorization"] = util.format('HMAC-SHA256 SignedHeaders=%s,Signature=%s', signedHeaders, signature);
